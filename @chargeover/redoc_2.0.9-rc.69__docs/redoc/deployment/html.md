...
Redoc is highly configurable, find a [full list of configuration options](/docs/redoc/config) on the dedicated page.
...
Check out the [list of options for theme configuration](/docs/redoc/config#theme-settings) and build up the configuration that suits your API needs.
...
* `specOrSpecUrl`: Either a JSON object with the OpenAPI definition or a file name/URL to the definition in JSON or YAML format.
* `options`: See the [configuration reference](/docs/redoc/config).
...
You can reference the Redoc script using either a link to the files hosted on a CDN or install to your `node-modules` folder. Using the CDN is the simplest option, but if you need to host in a closed environment or have requirements around external dependencies, it may be useful to self-host.
...
*(Yarn users can install the `redoc` package with `yarn`)*.
...
 Next pageOn this page[Configure Redoc](#configure-redoc)[Theme configuration](#theme-configuration)[Advanced options](#advanced-options)[The Redoc object](#the-redoc-object)[Self-host dependencies](#self-host-dependencies)
