...
Ghost(Pro) is a fully managed service with an end-to-end infrastructure and a deep integration with [Fastly](https://www.fastly.com/) as the edge content delivery network (otherwise known as a CDN). The use of Fastly with Ghost(Pro) sites ensures that your publication is protected with enterprise-grade security features, and enables us to serve content to your readers with the best performance possible.

Enabling a third-party CDN providers with your Ghost(Pro) site can likely lead to performance issues, and disables our ability to update your site properly when new content is published, and when changes are made to your content, routes, redirects and themes.

💡We will ask you to removed edge stacked services before we investigate any issues on Ghost(Pro).
