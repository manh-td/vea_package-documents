...
When a member’s payment fails in Stripe, what happens next will vary based on your Stripe settings.

When a member’s payment fails, their Stripe subscription status will change to `Past Due`, and Stripe will try to recover the payment based on your [retry settings](https://dashboard.stripe.com/revenue_recovery/retries). Members who are currently in a `Past Due` state will still have access to paid content on your site.

Once *all* retries to collect the payment have failed in Stripe, depending on your Stripe settings, the member’s subscription will be cancelled, preventing further paid access to content on your site.

When a member's subscription is cancelled due to non-payment, the member will be converted to a free member in Ghost, and will only have access public and members-only content on your site.
